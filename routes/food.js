...
router.get('/storage', isLoggedIn(),(req, res, next) => {
 ...
 // console.log('antes', req.session.currentUser.storage)
...
router.put('/storage/new', isLoggedIn(),async(req, res, next) => {
  try{
   ...
   let userStorage = req.session.currentUser.storage;
   
   //console.log(food)
   //console.log(userStorage)
   let resp =  userStorage.some(el => el.title === tempT);
   //console.log(resp)
   if(resp === true){
    userStorage.push(food)
    //ok 
    let newStorage = await User.findByIdAndUpdate(userID, 
     ...
     let newStorageUp= await User.findByIdAndUpdate(userID,
        {$push: {storage: {title: tempT, quantity: tempQ, unity: tempUn}}, new:true})//nao devolve atualizado
        ...
        //userStorage = req.session.currentUser.storage;
        req.session.currentUser.storage = await newStorageUp
        //console.log('depois',req.session.currentUser.storage.storage)
